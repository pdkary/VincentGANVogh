...
class ViewableTrainer(AbstractTrainer):
    ...
    def save_views(self,name):
        ...
        if self.D.view_layers:
            ...
            combined_source = np.append(image_batch,gen_images,axis=0)
            total_preds = np.append(disc_real_preds, disc_fake_preds,axis=0)
            total_views = [np.append(disc_real_views[i],disc_fake_views[i],axis=0) for i in range(len(disc_real_views))]
