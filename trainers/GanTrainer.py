...
class GanTrainer(GanTrainingConfig):
  ...
  def train_generator(self):
    ...
    with tf.GradientTape() as gen_tape:
      ...
      acc = 1 - sum(abs(fake_label - fake_out))/len(fake_label)
  ...
  def train_discriminator(self,training_images):
    ...
    with tf.GradientTape() as disc_tape:
      ...
      real_acc = 1 - sum(abs(real_label - real_out))/len(real_label)
      fake_acc = 1 - sum(abs(fake_label - fake_out))/len(fake_label)
